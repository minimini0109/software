...
<style> @import url('https://fonts.googleapis.com/css2?family=Montserrat:wght@300;500;700&display=swap'); .stApp { background-color: #f0fbff; font-family: 'Montserrat', sans-serif; color: #033f63; } .header-title { font-size: 64px; font-weight: 700; color: #0278ae; margin: 0; } .header-subtitle { font-size: 24px; color: #56cfe1; margin: 0; } </style>
""", unsafe_allow_html=True)

--- Header ---
st.markdown("""

<div style="text-align: center; padding: 20px;"> <p class="header-title">어퓨</p> <p class="header-subtitle">A few, just for you 💙</p> </div> <hr style="border:1px solid #cceafc"/> """, unsafe_allow_html=True)
--- Session 초기화 ---
...
st.session_state.selected_search_product = None # 검색에서 마지막으로 클릭한 제품 이름
if 'selected_search_ingredient' not in st.session_state:
st.session_state.selected_search_ingredient = None # 검색에서 마지막으로 클릭한 성분
...
"이름": name,
"종류": typ,
"가격": random.randint(price_range[typ], price_range[typ]),
"성분": ingredients,
"추천_피부톤": user["피부톤"],
"추천_피부타입": None,
"권장_민감도_max": 10,
...
current_type = user["피부타입"] if user["피부타입"] in skin_types else skin_types
current_tone = user["피부톤"] if user["피부톤"] in tones else tones
...
st.info(f"{ing_choice} → 장점: {info}, 주의: {info}")
